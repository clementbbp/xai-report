...
**Interpretation copy:**
> "The model does not sort articles into constructive and not constructive. It reads each one and places it on the same 1 to 5 scale our human raters used. The useful question is not whether it gets a verdict right, because it produces no verdict. The question is whether a higher score means more constructive journalism. It does. Across 567 test articles, the model's ordering tracks the order humans gave, with a correlation of 0.45. That is a moderate correlation. It is useful for sorting and prioritizing, but not strong enough to treat as a final answer.
>
> A yes or no label for constructiveness comes from two places, and neither is the model. The first is measurement. To check the model's accuracy, we compare it against a human answer, so an article a person rated 4 or 5 counts as constructive. The second is your own use. To act on the scores, you decide how far down the ranked list to read. You draw that line based on the time you have. The model does not draw it for you.
>
> The model is most accurate on conventional journalism. About 96 of every 100 clearly conventional articles score low. An editor using the scores as a first pass will see few of those articles near the top of the list.
>
> If you read down to a score of 3, you will find about 6 of every 10 constructive articles in the set, and about 4 of every 10 articles you read will be constructive. That is more than twice the rate you would get reading in no particular order. Setting the line higher means reading fewer articles and including fewer that do not fit. Setting it lower means finding more constructive articles but reading more that do not. The model's role is to place the stronger articles near the top of the list. It is least accurate in the middle of the scale, where constructive pieces that are heavily reported or negative in tone often land around 2.5 to 3.5 instead of higher. For that reason the score is still useful to read when it falls below a clear cutoff.
>
> To use the score, treat it as a ranked reading list, not a gate. Articles at 4 or above are strong candidates. Articles between 3 and 4 can be reviewed if time allows. Articles below 3 are unlikely to be constructive."

**Charts to embed:**
1. `assets/performance/score_ranking_lift.png` is the **lead visual** for this section. It answers what an editor gets by acting on the score. The chart sorts every article by model score and shows how many of each 10 reviewed are constructive, the deeper you read. Caption: "Sort articles by score and read the highest first. The top 20% of the list contains constructive journalism at about 4 in 10, more than twice the rate of reading in no particular order (about 2 in 10). The line slopes downward because the strongest articles sit at the top, so the deeper you read, the more ordinary pieces you reach. The downward slope shows the ranking is working, not failing."
   - **Pull-stat to render beside the chart** (large, gold accent): **2.2×** with the line "Read the top 20% of articles by score and you find constructive journalism at more than twice the rate of reading at random." The pull-stat carries the message at a glance. The chart is the evidence for anyone who looks closer.
2. `assets/performance/calibration_by_cscore.png` is a supporting visual. Caption: "Mean model score by human rating. Each step up in human rating corresponds to a higher model score, and the ordering is consistent across all five levels."
3. Do NOT embed `ordinal_error_distribution.png`. It frames the model as a binary classifier (correct vs wrong at a fixed threshold), which contradicts this section's ranking-tool framing.
4. Do NOT embed `confusion_matrix.png`. It is generated at a fixed score threshold (≥ 4.0) that produces only 17% recall, which is inconsistent with the threshold-agnostic framing of this section and would mislead editors.
