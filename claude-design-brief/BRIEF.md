...
**Charts to embed:**
1. `assets/performance/score_ranking_lift.png` — **lead visual** for this section. It answers the editor's real question: "what do I get if I act on the score?" The chart sorts every article by model score and shows how many of each 10 reviewed are actually constructive, the deeper you read. Caption: "Sort articles by score and read the highest first. The top 20% of the list contains constructive journalism at about 4 in 10 — more than twice the rate of reading in no particular order (about 2 in 10). The line slopes downward because the strongest articles sit at the top: the deeper you read, the more ordinary pieces you reach. That downward slope is the model working, not failing."
   - **Pull-stat to render beside the chart** (large, gold accent): **2.2×** — "Read the top 20% of articles by score and you'll find constructive journalism at more than twice the rate of reading at random." The pull-stat carries the message at a glance; the chart is the evidence for anyone who looks closer.
2. `assets/performance/calibration_by_cscore.png` — supporting visual. Caption: "Mean model score by human rating. Each step up in human rating corresponds to a higher model score — the ordering is consistent across all five levels."
