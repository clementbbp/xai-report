...
- Green `rgba(0,180,0,α)` = words/sentences pushing toward constructive
- Red `rgba(200,0,0,α)` = words/sentences pushing toward conventional
- Alpha encodes attribution strength (stronger = more opaque)
- Grey `rgba(180,180,180,α)` = beyond model reading window (no attribution)
...
>
> **Chart caption:** Green bars show sentences that pushed the model toward a constructive verdict. The opening sentence — "a 'graveyard' under construction is raising the hopes of climate experts" — is the single strongest signal: it reframes industrial infrastructure as a hopeful solution. Sentences about the cross-border commercial agreement and new pipeline announcements follow. Red bars (sentences pushing toward conventional) are short, generic, or express skepticism about the technology.
...
>
> **Chart caption:** Most sentences are neutral to weakly conventional. The article describes a product launch — a new TV show — with no problem framed, no community impact, no forward-looking solution. The model correctly reads this as conventional entertainment coverage.
...
> "Signal strength is measured within each article — bars are not directly comparable across articles. Article confidence is the model's overall verdict on the full piece."
